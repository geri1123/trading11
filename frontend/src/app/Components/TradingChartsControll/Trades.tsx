    ...
    <div className="w-full p-2 lg:p-2 xl:p-2 2xl:p-3 flex flex-col bg-black-300 rounded-20">
      ...
      <div className="top lg:flex xxl:flex xl:flex items-center grid grid-cols-1 w-full border-b border-gray-500 lg:pb-3 xxl:pb-3 xl:pb-2 xxl:justify-between xl:justify-between lg:justify-between">
        ...
        <div className="bg-black-700 p-2 flex justify-between lg:gap-4 xl:gap-4 xxl:gap-4 rounded-xl">
          {(["Position", "Pending", "Closed Position"] as const).map((tab) => (
            <button
              key={tab}
              className={`tab-btn ${
                activeTab === tab ? "bg-black-300" : "bg-black-700"
              } rounded-lg w-full px-2 py-2 lg:px-4 lg:py-2 xxl:px-4 xxl:py-2 2xl:px-4 2xl:py-2 xl:px-4 xl:py-1 flex items-center gap-2 justify-center`}
              onClick={() => setActiveTab(tab)}
            >
              <p className="whitespace-nowrap text-white md:text-[13px] sm:text-[13px] xsm:text-[8px] text-[13px] lg:text-base xl:text-sm xxl:text-base 2xl:text-base leading-none">
                {tab}
              </p>
              <div className="flex items-center justify-center w-5 h-5 rounded-md bg-black-200">
                <p className="text-white lg:text-[12px] xl:text-[12px] 2xl:text-[12px] text-[10px] leading-none">
                  {counts[getStatusForTab(tab)]}
                </p>
              </div>
            </button>
          ))}
          <div className="w-full text-center flex items-center justify-end lg-hidden xl:hidden xxl:hidden">
            <Image
              src="/Images/Icons/close-all-small.svg"
              onClick={handleOpenDialog}
              className="bg-black-200 p-2 rounded-lg lg:hidden xl:hidden xxl:hidden"
              alt="close-all-small.svg"
              width={20}
              height={20}
            />
          </div>
        ...
        <div className="flex items-center gap-6 ">
          {activeTab === "Closed Position" && (
            <div className="lg:flex xl:flex xxl:flex 2xl:flex hidden gap-6">
              <div className="flex items-start flex-col">
                ...
                <select
                  className="text-[9px] h-6 p-0 mt-1 px-3 text-white focus:ring-0 border-none outline-none cursor-pointer bg-black-700 font-medium"
                  value={summaryFilter}
              ...
              <div className="flex gap-6">
                ...
                <div>
                  <p className="text-xs text-white font-medium text-opacity-50">
                    Withdrawal
                  </p>
                  <div>
                    <p className="text-[10px] text-white font-medium leading-none text-end mt-3">
                      -USD$15
                    </p>
                  </div>
                </div>
          ...
          <button
            onClick={handleOpenDialog}
            className="lg:bg-transparent lg:border xl:border xl: border-white lg:border-white py-[5px] px-[3px] rounded-lg xl:px-5 xxl:px-5  lg:px-5 lg:rounded-lg xl:rounded-lg xxl:rounded-lg flex items-center gap-2 text-base text-white"
          >
            <p className="hidden xl:text-sm lg:flex xl:flex xxl:flex">
              Close all trades
            </p>
          </button>
        </div>
      ...
      <div className="mt-2 text-white">
        ...
        {activeTab === "Pending" && (
          <>
            <div className="hidden xl:block">{/* <PendingContent /> */}</div>
            <div className="block xl:hidden">
