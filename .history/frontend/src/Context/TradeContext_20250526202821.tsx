...
//     handleCloseTrade: (id: string | number) => Promise<void>;
// }
...
export interface Trade {
  ...
  orderId?: string | number;
  positionId?: string | number;
  [key: string]: unknown; // FIX: replaced 'any' with 'unknown'
}
...
export const TradeProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  const fetchActiveTabTrades = useCallback(async () => {
    ...
    setLoading(false);
  }, [activeTab, getStatusForTab]); // FIX: added getStatusForTab as a dependency
  ...
  const handleCloseTrade = useCallback(async (id: string | number) => {
    ...
    try {
      ...
      await response.json(); // FIX: removed unused 'data'
      toast.success(`Trade ${id} closed successfully!`);
      await refreshData();
    } catch (error) {
      toast.error('Failed to close trade.');
