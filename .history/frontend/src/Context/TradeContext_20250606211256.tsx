...
export const TradeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchTrades = async (type: TradeType, page: number = 0) => {
    try {
      ...
      switch (type) {
        case 'OPEN':
          url = `/api/tables/positions/open?page=${page}`;
          break;
        case 'CLOSED':
          url = `/api/tables/positions/closed?page=${page}`;
          break;
        case 'PENDING':
          // Note: You might need to create this endpoint or use a different one
          url = `/api/tables/positions/pending?page=${page}`;
          break;
        default:
    ...
    const fetchAllCounts = async () => {
      try {
        const [openRes, closedRes] = await Promise.all([
          axios.get<ApiResponse>('/api/tables/positions/open?page=0'),
          axios.get<ApiResponse>('/api/tables/positions/closed?page=0'),
          // Add pending if the endpoint exists
          // axios.get<ApiResponse>('/api/tables/positions/pending?page=0'),
        ]);
