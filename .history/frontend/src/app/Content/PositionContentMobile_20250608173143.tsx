...
const PositionContentMobile: React.FC= ({ positionData }) => {
  ...
  return (
    ...
    ">
      {positionData.map((row) => (
        <div key={row.id} className='bg-black-700 rounded-2xl px-3 py-3'>
              ...
              <div className='flex flex-col gap-3 align-center text-start'>
                ...
                <div className='flex items-center text-center justify-center gap-2'>
                  <div className='flex gap-2'>
                    ...
                    <p className='text-[11px] font-semibold'>{row.size}</p>
                  </div>
                  <div className='w-[1px] h-full bg-gray-200'></div>
                  <div className='flex items-center justify-start gap-1'>
                    <p className='text-[11px] text-gray-400 font-semibold'>Size</p>
                    <p className={`text-[10px] ${openId === row.id ? "hidden" : "flex"}`}>{row.entryMarket}</p>
                  </div>
                </div>
              </div>
            </div>
