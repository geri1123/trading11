...
const ClosedPositionMobile: React.FC = () => {
  ...
  // Only show closed trades (assume context provides only closed trades when on Closed Position tab)
  ...
  if (!closedData || closedData.length === 0) return <div className="text-center py-8">No closed positions found</div>;
    ...
    >
      ...
      <div className="flex justify-around gap-6 w-full">
        <div className="flex items-start flex-col">
          <p className="text-xs font-medium text-white text-opacity-50">Summary</p>
          <div className="bg-black-700 w-[120px] h-[34px] p-1 rounded-lg flex items-center justify-center mt-1">
            <select className="w-full text-xs text-white focus:ring-0 border-none outline-none cursor-pointer bg-black-700 font-medium" disabled>
              <option value="all">All</option>
            </select>
          </div>
        </div>
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-3">
            <Image src="/Images/Icons/decrease.svg" alt="decrease-icon" width={12} height={12} />
            {/* Example: total profit/loss sum */}
            <p className="text-[10px] text-red-400 leading-none">
              {formatNumber(closedData.reduce((sum, trade) => sum + (trade.profitLoss ?? 0), 0), 2)}
            </p>
          </div>
        </div>
        <div>
          <p className="text-xs font-medium text-white text-opacity-50">Size</p>
          <div>
            {/* Example: total size sum */}
            <p className="text-[10px] font-medium text-white leading-none mt-3">
              {formatNumber(closedData.reduce((sum, trade) => sum + (trade.lotSize ?? 0), 2), 2)}
            </p>
          </div>
        </div>
        <div>
          <p className="text-xs text-white font-medium text-opacity-50">Withdrawal</p>
          <div>
            {/* Example: fake value, adjust if you have withdrawal data */}
            <p className="text-[10px] text-white font-medium leading-none text-end mt-3">-USD$15</p>
          </div>
        </div>
      </div>
      {closedData.map((row) => (
        ...
        >
          <div className="up flex justify-between">
              ...
              <div className="flex flex-col gap-3 align-center justify-between text-start">
                ...
                <div className="flex items-center text-center justify-center gap-2">
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <p className="text-[12px] font-semibold">{formatNumber(row.lotSize, 2)}</p>
                  </div>
                  <div className="w-[1px] h-full bg-gray-200"></div>
                  <div className="flex items-center justify-center gap-1.5">
                    <p className="text-[11px] text-gray-400 font-semibold">Type</p>
            ...
            <div className="left flex items-center justify-center flex-col gap-2">
              <p className={'bg-green-800 flex py-1 bg-opacity-50 px-2 rounded-md text-green-200 text-xs'}>
                <Image src="/Images/Icons/increase.svg" alt="" width={12} height={12} /> {formatNumber(row.margin, 2)}
              </p>
              <div className="flex">
                <p>Market</p>
              </div>
          ...
          >
            <div className="bottom py-2 border-t mt-4 gap-3 flex flex-col border-gray-400">
              <div className="flex justify-between">
                <div className="text-start flex flex-col gap-[3px]">
                  <p className="text-sm font-semibold p-0 m-0 font-base text-gray-400">Entry Price</p>
                  <p className="text-sm font-semibold text-white">{formatNumber(row.entryPrice, 4)}</p>
                  <p className="text-[10px] text-white">{formatDate(row.openedAt)} </p>
                </div>
                <div className="flex flex-col items-start justify-between ">
                  <p className="text-sm font-semibold p-0 m-0 font-base text-gray-400">Exit Price</p>
                  <p className="text-sm font-semibold text-white">{formatNumber((row as any).closePrice, 4)}</p>
                  <p className="text-[10px] text-white">{formatDate((row as any).closedAt) ?? ''} </p>
                </div>
              </div>
              <div className="w-full h-[1px] bg-gray-500"></div>
              <div className="flex justify-between">
                <div className="flex flex-col items-start gap-[3px]">
                  <p className="text-sm font-semibold p-0 m-0 font-base text-gray-400">Order ID</p>
                  <p className="flex items-center font-semibold text-sm">{(row as any).orderId ?? '-'}</p>
                  <p className="text-[10px] text-white font-semibold">
                    Position ID: {row.positionId} | Swap/Fee : USD${formatNumber(row.swap, 2)}/{formatNumber(row.fee, 2)}
                  </p>
                </div>
                <div className="flex items-start flex-col justify-between">
                  <p className="text-sm font-semibold">
                    <span className="text-gray-400">SL</span> : {formatNumber(row.stopLoss, 4)}
                  </p>
                  <p className="text-sm font-semibold">
                    <span className="text-gray-400">TP</span> : {formatNumber(row.takeProfit, 4)}
                  </p>
                  <p className="text-sm font-semibold">
                    <span className="text-gray-400">P/L</span> : <span className={parseFloat(String(row.profitLoss)) < 0 ? 'text-red-500' : 'text-green-500'}>{formatNumber(row.profitLoss, 2)}</span>
                  </p>
                </div>
              </div>
            </div>
