    ...
    <div className="bg-gray-900 rounded-lg shadow-xl overflow-hidden max-h-96">
          ...
          <tbody className="divide-y divide-gray-700">
            {trades.map((row, index) => (
              <tr key={row.positionId || index} className="hover:bg-gray-800 transition-colors duration-150">
                ...
                <td className="text-center text-gray-200 text-xs">{formatNumber(row.orders.lotSize, 2)}</td>
      ...
      <div className="lg:hidden">
        <div className="space-y-4 p-4">
          {trades.map((row, index) => (
            <div key={row.positionId || index} className="bg-gray-800 rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-gray-400">Size:</span> <span className="text-white ml-2">{formatNumber(row.lotSize, 2)}</span></div>
                <div><span className="text-gray-400">Entry:</span> <span className="text-white ml-2">{formatNumber(row.entryPrice, 5)}</span></div>
                <div><span className="text-gray-400">Margin:</span> <span className="text-white ml-2">{formatNumber(row.margin, 2)}</span></div>
                <div><span className="text-gray-400">P&L:</span> 
                  <span className={`ml-2 font-bold ${row.profitLoss < 0 ? 'text-red-400' : 'text-green-400'}`}>
                    {row.profitLoss > 0 ? '+' : ''}{formatNumber(row.profitLoss, 2)}
                  </span>
                </div>
              </div>

              <div className="text-xs text-gray-400 pt-2 border-t border-gray-700">Opened: {formatDate(row.openedAt)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Dialogs */}
      {isOpen && activeDialog === 'partial' && selectedData && (
        <div className={`fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          <div ref={dropdownref} className={`${isClosing ? "animate-slideUp" : "animate-slideDown"}`}>
            <PartialDialog data={selectedData} onClose={closeDropdown} onAfterClose={handleAfterClose} />
          </div>
        </div>
      )}
      {isOpen && activeDialog === 'Modify' && selectedData && (
