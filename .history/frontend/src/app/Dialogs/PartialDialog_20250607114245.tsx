...
export interface PartialDialogProps {
  data: any;
  onClose: () => void;
  onAfterClose?: () => void; // Callback to refresh list after close
}

const PartialDialog: React.FC<PartialDialogProps> = ({ data, onClose, onAfterClose }) => {
  const [value, setValue] = useState<number>(0.00);
  ...
  // Use this if your backend supports partial close by lot (otherwise, just full close)
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await closePosition(data.positionId); // Add partial logic as needed
  ...
  return (
    ...
    >
      ...
      <div className='flex w-full gap-3'>
        <button onClick={onClose} className='w-1/3 border-1 border-white py-2 rounded text-sm text-white'>Cancel</button>
        <button
          className='w-2/3 text-black bg-blue-400 text-sm rounded'
          disabled={submitting}
          onClick={handleSubmit}
        >
          {submitting ? 'Closing...' : 'Submit'}
        </button>
      </div>
    </div>
  );
