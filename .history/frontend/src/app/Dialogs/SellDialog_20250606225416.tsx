...
import { createOrder, CreateTradeData } from "@/api/apiTrades";
...
const SellDialog: React.FC<SellDialogProps> = ({ onClose, value, data }) => {
  ...
  const handleSell = async (): Promise<void> => {
    ...
    try {
      // Use correct keys and types from CreateTradeData
      const tradeData: CreateTradeData = {
        instrument: data.pair,
        side: "SELL",
        lotSize: value,
        stopLoss: 1,    // Provide sensible defaults or gather from user
        takeProfit: 1,  // Provide sensible defaults or gather from user
      };
      ...
      if (err instanceof Error) {
        toast.error(`Failed to create trade: ${err.message}`);
      } else {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className='w-full border-1 border-white bg-black-700 py-2 rounded'>
        <p className='text-white text-sm'>Sell details</p>
      </div>
      ...
      <div className='w-full flex flex-col gap-3 items-start'>
        <p className='text-white text-xs'>Market sell price</p>
        <div className='w-full bg-black-700 border-1 border-white border-opacity-50 py-2 px-3 rounded'>
          <p className='text-white text-sm'>{data.bid.toFixed(5)}</p>
        </div>
      </div>
      ...
      <p className='text-red-400 text-xs'>
        You are going to sell {value} lot{value !== 1 ? "s" : ""}.
      </p>
      ...
      <div className='flex w-full gap-3'>
        ...
        <button
          onClick={handleSell}
          disabled={loading}
          className='w-2/3 text-black bg-red-400 text-sm rounded py-2 hover:bg-red-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
        >
