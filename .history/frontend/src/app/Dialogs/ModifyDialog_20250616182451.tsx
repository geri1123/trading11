...
const ModifyDialog: React.FC<ModifyDialogProps> = ({ data, onClose }) => {
...
const handleSubmit = async () => {
  ...
  try {
    await updateStopLossTakeProfit({
      positionId: data.PositionID || data.id,
      stopLoss: firstInputValue ? parseFloat(firstInputValue) : undefined,
      takeProfit: secondInputValue ? parseFloat(secondInputValue) : undefined,
    });
    onClose(); // Close dialog on success
  } catch (error) {
    console.error('Failed to update stop loss/take profit:', error);
    alert('Failed to update SL/TP. Please try again.');
  }
};
  useEffect(() => {
    if (data) {
  ...
  className={`w-2/3 text-black text-sm rounded ${data.side === 'Buy' ? 'bg-green-400' : 'bg-red-400'}`}
>
  Modify
</button>
      </div>
