...
const MainPage: React.FC = () => {
  ...
  // Update selected data when live data changes
  useEffect(() => {
    if (selectedData && liveData[selectedData.pair]) {
      const liveUpdate = liveData[selectedData.pair];
      if (liveUpdate.a !== undefined && liveUpdate.b !== undefined) {
        setSelectedData(prev => prev ? {
          ...prev,
          ask: liveUpdate.a!,
          bid: liveUpdate.b!,
          spread: liveUpdate.spread || 0
        } : null);
      }
    }
  }, [liveData, selectedData?.pair]);

  const handlePairSelection = (data: SelectedData) => {
