...
const MainPage: React.FC = () => {
  ...
  // Update selected data when live data changes for the selected pair
  useEffect(() => {
    if (selectedData && liveData[selectedData.pair]) {
      const liveDataForPair = liveData[selectedData.pair];
      if (liveDataForPair.a !== undefined && liveDataForPair.b !== undefined) {
        setSelectedData(prev => prev ? {
          ...prev,
          ask: liveDataForPair.a!,
          bid: liveDataForPair.b!,
          spread: liveDataForPair.spread || 0
        } : null);
      }
    }
  }, [liveData, selectedData?.pair]);

  const handleSelectData = useCallback((data: SelectedData) => {
    // Update with live data if available
    const liveDataForPair = liveData[data.pair];
    if (liveDataForPair && liveDataForPair.a !== undefined && liveDataForPair.b !== undefined) {
      setSelectedData({
        ...data,
        ask: liveDataForPair.a,
        bid: liveDataForPair.b,
        spread: liveDataForPair.spread || data.spread
      });
    } else {
        ...
        <div className="h-full flex flex-col gap-3">
          ...
          <div className='all-tradingchart h-full grid grid-cols-[2fr_1fr] gap-3'>
            ...
            <div className='flex flex-col h-full gap-2'>
              <ChartControll onSelect={handleSelectData} liveData={liveData} />
