...
const ChartControll: React.FC<ChartControllProps> = ({ onSelect }) => {
  ...
  const handlePairSelect = (pair: string) => {
    const data = liveData[pair];
    console.log('Selecting pair:', pair, 'with data:', data); // Debug log
    ...
    if (data && typeof data.a === "number" && typeof data.b === "number") {
      ...
      console.log('Calling onSelect with:', selectedData); // Debug log
      onSelect(selectedData);
    } else {
      console.log('Data not available for pair:', pair); // Debug log
    }
  };
