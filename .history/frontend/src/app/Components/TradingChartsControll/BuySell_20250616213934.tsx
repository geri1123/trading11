...
const calculateMargin = (price: number) => {
  const leverageValue = user?.leverage ?? 100; // Default to 100 if not provided
  const positionSize = value * lotSize;        // Trade size in units (e.g., 0.01 lots = 1,000)

  const margin = (positionSize * price) / leverageValue;

  // Margin as percentage of user's balance
  const marginPercentageOfBalance = userBalance > 0 
    ? (margin / userBalance) * 100 
    : 0;

  return {
    amount: margin.toFixed(2), // e.g., "$11.57"
    percentage: marginPercentageOfBalance.toFixed(2), // e.g., "0.10" (% of balance)
  };
};
