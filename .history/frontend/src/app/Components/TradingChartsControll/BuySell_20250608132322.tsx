...
// //           <img src={currencyFlags.base} className='h-3.5 w-3.5 rounded-full' alt="base currency" />
// //           <p className="leading-none text-white text-14px xl:text-[12px] pt-[2px] font-semibold">
// //             {selectedData.pair}
// //           </p>
// //           <img src={currencyFlags.quote} className='h-3.5 w-3.5 rounded-full' alt="quote currency" />
// //         </div>
// //         <button
...
const BuyDialog: React.FC<BuyDialogProps> = ({ onClose, value, data }) => {
  ...
  const handleBuy = async (): Promise<void> => {
    ...
    setLoading(true);
    try {
      // Use actual values from user input and selected data
      const tradeData: CreateOrderData = {
        instrument: data.pair,
        side: "BUY",
        lotSize: value,
        stopLoss: 0.1, // Replace with actual input if you have fields for these
        takeProfit: 0.1,
        status: "OPEN",
      ...
      toast.error(err.message || "Failed to create trade. Please check input or try again later.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!data) {
    return (
      <div className='bg-black-300 w-screen md:w-[450px] p-5 flex flex-col items-center gap-3 justify-center rounded-20'>
  ...
  return (
    <div
      className='bg-black-300 w-screen md:w-[450px] p-5 flex flex-col items-center gap-3 justify-center rounded-20'
