...
//   onSelect: (element: SelectedData | null) => void;
// }
...
//  useEffect(() => {
//   // Auto-select the first available pair if not already done
...
//     if (typeof data.a === "number" && typeof data.b === "number" && typeof data.spread === "number") {
//       onSelect({
//         pair: selectedPair,
//         ask: data.a,
//         bid: data.b,
...
//     }
//   }
// }, [liveData, filteredPairs, onSelect, hasAutoSelected, selectedPair]);
//   useEffect(() => {
...
//               console.log("Received message for", pair, message.body);
...
const ChartControll: React.FC<ChartControllProps> = ({ onSelect }) => {
  ...
  // Auto-select the first available pair only once
  // useEffect(() => {
  //   if (!hasAutoSelected && Object.keys(liveData).length > 0) {
  //     for (const pair of filteredPairs) {
  //       const data = liveData[pair];
  ...
  //     }
  //   }
  // }, [liveData, filteredPairs, hasAutoSelected]);

  // Whenever selectedPair or its data changes, call onSelect
  useEffect(() => {
    if (selectedPair && liveData[selectedPair]) {
      const data = liveData[selectedPair];
      if (typeof data.a === "number" && typeof data.b === "number" && typeof data.spread === "number") {
  ...
  }, [selectedPair, liveData, onSelect]);
 useEffect(() => {
    const token = Cookies.get("token");
    if (!token) return;
