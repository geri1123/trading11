...
// ...[same arrays and interfaces as before]...
...
const ChartControll: React.FC<ChartControllProps> = ({ onSelect }) => {
  ...
  // --- COOKIES: the browser will send cookies automatically with the websocket handshake ---
  // As long as the domain and path are correct, and the user is logged in, cookies will be sent.
  // No extra frontend code is needed for cookies. Do NOT use withCredentials or headers in the native WebSocket API.
  // If you are using a custom proxy or have CORS issues, solve them server-side.

  useEffect(() => {
    const client = new Client({
      webSocketFactory: () =>
        new WebSocket(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/ws/forex`
        ),
      reconnectDelay: 5000,
      onConnect: () => {
        allPairs.forEach((pair) => {
