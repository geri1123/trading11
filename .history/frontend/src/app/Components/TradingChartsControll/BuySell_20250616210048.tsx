...
const calculateMargin = (price: number) => {
  ...
  const margin = (positionSize * price) / leverage;
  const marginPercentage = (margin / (positionSize * price)) * 100;
  return {
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="w-6/12 flex flex-col gap-1">
          <p className="text-sm xl:text-sm xxl:text-sm font-semibold text-white">
            Margin
            <span className="pl-1 xl:text-sm text-[11px] font-medium">{marginInfo}</span>
          </p>
