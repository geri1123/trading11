...
// const BuySell: React.FC<BuySellProps> = ({ data , refreshData }) => {
...
interface BuySellProps {
  data?: SelectedData | null; 
  refreshData?: () => void;
  onTradeSuccess?: () => void; // Add this prop to handle trade success
}
...
const BuySell: React.FC<BuySellProps> = ({ data, refreshData, onTradeSuccess }) => {
        ...
        <div className={`fixed h-screen top-0 left-0 w-full h-full bg-black-700 bg-opacity-70 flex items-center justify-center z-50 ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          ...
          >
            <BuyDialog 
              ...
              data={selectedData}
              onTradeSuccess={handleTradeSuccess} // Pass the success handler
            />
          </div>
        </div>
      )}
        ...
        <div className={`fixed h-full h-screen top-0 left-0 w-full bg-black-700 bg-opacity-70 flex items-center justify-center z-50 ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          ...
          >
            <SellDialog 
              ...
              data={selectedData} 
              onClose={closeDropdown}
              onTradeSuccess={handleTradeSuccess} // Pass the success handler to SellDialog too
            />
          </div>
        </div>
      )}
