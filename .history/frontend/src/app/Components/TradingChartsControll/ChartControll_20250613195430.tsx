...
const ChartControll: React.FC<ChartControllProps> = ({ onSelect }) => {
  ...
  useEffect(() => {
    console.log("Effect: auto-select check", { hasAutoSelected, liveData, filteredPairs });
    if (!hasAutoSelected && Object.keys(liveData).length > 0) {
      for (const pair of filteredPairs) {
        ...
        if (data && typeof data.a === "number" && typeof data.b === "number" && typeof data.spread === "number") {
          ...
          console.log("Auto-selecting pair:", pair, data);
          onSelect({
            pair,
            ask: data.a,
            bid: data.b,
  ...
  useEffect(() => {
    ...
    console.log("ChartControll mount. Token:", token, "API URL:", apiUrl);

    if (!token) {
      console.warn("No token found in cookies. Quotes will not load.");
      return;
    }
    if (!apiUrl) {
    ...
    const wsUrl = `${apiUrl}/ws/forex?token=${token}`;
    ...
    const client = new Client({
      ...
      reconnectDelay: 5000,
      onConnect: () => {
        console.log("STOMP connected. Subscribing to all pairs...");
        allPairs.forEach((pair) => {
          const topic = `/topic/forex.${pair.toLowerCase().replace("/", ".")}`;
          client.subscribe(topic, (message) => {
            try {
              const data: LivePairData = JSON.parse(message.body);
              console.log(`[STOMP] Received for ${pair}:`, data);
              setLiveData((prev) => ({ ...prev, [pair]: data }));
            } catch (err) {
              console.error("Error parsing STOMP message for", pair, err, message.body);
            }
          });
        });
      },
    ...
    return () => {
      ...
      console.log("ChartControll cleanup: STOMP deactivated.");
    };
  }, []);

  const handlePairSelect = (pair: string) => {
    const data = liveData[pair];
    console.log("User selected pair:", pair, "Data:", data);
    if (data && typeof data.a === "number" && typeof data.b === "number") {
      ...
      console.warn("No valid data available for selected pair:", pair, data);
    }
  };

  // Debug: show liveData in UI if needed
