...
const calculateMargin = () => {
  const tradeSize = value; // lot size, e.g., 0.01
  const price = selectedData.ask || selectedData.bid; // current market price
  const contractSize = 100000; // standard lot in forex

  const positionValue = tradeSize * contractSize * price;
  const marginRequired = positionValue / leverage;

  const marginPercent = ((marginRequired / accountBalance) * 100).toFixed(2);

  return {
    marginRequired: marginRequired.toFixed(5),
    marginPercent,
  };
};
const { marginRequired, marginPercent } = calculateMargin();
  useEffect(() => {
    setValue(0.01);
  }, [selectedData]);
