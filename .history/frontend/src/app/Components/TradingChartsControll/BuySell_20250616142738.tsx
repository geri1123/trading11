...
// Default fallback if no data is provided
const DEFAULT_SELECTED: SelectedData = {
  pair: 'GBP/USD',
  ask: 1.10537,
  bid: 1.10532,
...
const BuySell: React.FC<BuySellProps> = ({ data }) => {
  ...
  return (
    <div className="w-full xl:p-3 xxl:p-3.5 p-2.5 overflow-hidden bg-black-300 rounded-20">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          <Image src={currencyFlags.base} alt="base currency" className="h-3.5 w-3.5 rounded-full" width={16} height={16} />
          <p className="text-white text-14px xl:text-[12px] pt-[2px] font-semibold">
            {selectedData.pair}
          </p>
          <Image src={currencyFlags.quote} alt="quote currency" className="h-3.5 w-3.5 rounded-full" width={16} height={16} />
        </div>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="w-6/12 flex flex-col gap-1">
          <p className="text-sm font-semibold text-white">
            Margin <span className="pl-1 text-[11px] font-medium">$11.04920 / 0.11%</span>
          </p>
          <div className="w-full flex items-center p-1 mt-1 bg-black-700 border rounded-lg border-green-10">
            <button onClick={decreaseValue} disabled={value <= 0.01} className="w-1/6 flex justify-center">
            ...
            />
            <button onClick={increaseValue} className="w-1/6 flex justify-center">
        ...
        <div className="w-3/12 flex flex-col gap-1">
          <p className="text-sm font-medium text-green-300 text-end">
            {selectedData.ask?.toFixed(5) ?? '-'}
          </p>
          <button
            onClick={() => handleOpenDialog('buy')}
            className="w-full p-3 text-base font-normal rounded-lg mt-2 bg-green-400 hover:bg-green-500"
          >
            Buy
          </button>
        </div>
      ...
      {/* Dialogs */}
      {isOpen && activeDialog === 'extand' && (
        <div className={`fixed inset-0 bg-black-700 bg-opacity-70 z-50 ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          <div ref={dropdownref} className={`z-50 fixed bottom-0 left-0 lg:relative ${isClosing ? 'animate-slideUp' : 'animate-slideDown'}`}>
            <Buysellextend data={selectedData} onClose={closeDropdown} dropdownref={dropdownref} />
          </div>
        </div>
      )}
      ...
      {isOpen && activeDialog === 'sell' && (
        <div className={`fixed inset-0 bg-black-700 bg-opacity-70 z-50 ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          <div ref={dropdownref} className={`z-50 fixed bottom-0 left-0 lg:relative ${isClosing ? 'animate-slideUp' : 'animate-slideDown'}`}>
            <SellDialog value={value} onClose={closeDropdown} data={selectedData} />
          </div>
        </div>
      )}
    </div>
