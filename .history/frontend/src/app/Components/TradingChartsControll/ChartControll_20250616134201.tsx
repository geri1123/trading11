// 1. First, update MainPage.tsx to manage live data state
"use client";
...
// 2. Update ChartControll.tsx to pass live data to parent
...
interface ChartControllProps {
  ...
  onLiveDataUpdate: (data: LiveDataMap) => void; // Add this prop
}

const ChartControll: React.FC<ChartControllProps> = ({ onSelect, onLiveDataUpdate }) => {
  const [liveData, setLiveData] = useState<LiveDataMap>({});
  ...
  // Pass live data to parent whenever it updates
  useEffect(() => {
...
//   // Update BuySell component when live data changes for the selected pair
//   useEffect(() => {
//     if (selectedPair && liveData[selectedPair]) {
//       const data = liveData[selectedPair];
//       if (data && typeof data.a === "number" && typeof data.b === "number") {
//         onSelect({
//           pair: selectedPair,
//           ask: data.a,
...
//       }
//     }
//   }, [liveData, selectedPair, onSelect]);
...
//   const handlePairSelect = (pair: string) => {
//     const data = liveData[pair];
//     console.log('Selecting pair:', pair, 'with data:', data);
...
//         dayLow: 0
//       };
//       console.log('Calling onSelect with:', selectedData);
//       onSelect(selectedData);
//     } else {
//       console.log('Data not available for pair:', pair);
//     }
//   };
...
//                   const data = liveData[pair];
//                   const isSelected = selectedPair === pair;
//                   const hasValidData = data && typeof data.a === "number" && typeof data.b === "number";
                  
//                   return (
//                     <tr
//                       key={i}
//                       onClick={() => hasValidData && handlePairSelect(pair)}
//                       className={`cursor-pointer hover:bg-black-500 text-xs font-medium text-white transition-colors ${
//                         isSelected ? 'bg-black-500 border-l-2 border-green-400' : 'bg-black-700'
