...
interface BuySellProps {
  ...
  liveData?: Record<string, { a?: number; b?: number; spread?: number }>;
}

const BuySell: React.FC<BuySellProps> = ({ data, liveData = {} }) => {
  ...
  // Get current live data for the selected pair
  const getCurrentData = (): SelectedData | null => {
    if (!data) return null;
    
    const currentLiveData = liveData[data.pair];
    if (currentLiveData) {
      return {
        ...data,
        ask: currentLiveData.a ?? data.ask,
        bid: currentLiveData.b ?? data.bid,
        spread: currentLiveData.spread ?? data.spread,
      };
    }
    return data;
  };
  ...
  // Show "No trades open" message if no data is available
  if (!selectedData) {
    return (
      <div className={`w-full xl:p-3 xxl:p-3.5 p-2.5 overflow-hidden bg-black-300 rounded-20`}>
        <div className="flex items-center justify-center py-8">
          <div className="text-center">
            ...
            />
            <p className="text-white text-lg font-medium mb-2">No trades open</p>
