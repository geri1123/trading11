...
const BuySell: React.FC<BuySellProps> = ({ data }) => {
  ...
  useEffect(() => {
    setValue(0.01); // Reset margin value on pair change
  }, [selectedData.pair]);
  ...
  const getCurrencyFlags = (pair: string) => {
    const [base = 'gbp', quote = 'usd'] = pair?.split('/') || [];
    const flag = (code: string) => `/Images/flags/${code.toLowerCase()}.svg`;
    return { base: flag(base), quote: flag(quote) };
  };
    ...
    <div className="w-full xl:p-3 xxl:p-3.5 p-2.5 overflow-hidden bg-black-300 rounded-20">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <p className="text-white text-14px font-semibold">{selectedData.pair}</p>
          <Image src={currencyFlags.quote} className='h-3.5 w-3.5 rounded-full' alt="quote" width={16} height={16} />
        </div>
        <button onClick={() => handleOpenDialog('extand')} className="p-2 rounded-lg bg-black-700">
          <Image src="/Images/Icons/chevron-down.svg" alt="chevron" className="rotate-180" width={16} height={16} />
        </button>
      </div>

      <div className="w-full h-[1px] bg-white bg-opacity-40 my-1.5" />

      {/* Buy / Sell Area */}
      <div className="flex items-start justify-between gap-4">
        <div className="w-3/12 flex flex-col gap-1 ">
          <p className="text-sm font-medium text-red-400">{selectedData.bid.toFixed(5)}</p>
          <button onClick={() => handleOpenDialog('sell')} className="w-full p-3 text-white rounded-lg mt-2 bg-red-400 hover:bg-red-500">
            Sell
          </button>
        </div>

        <div className="w-6/12 flex flex-col gap-1">
          <p className="text-sm font-semibold text-white">Margin
            <span className="pl-1 text-[11px] font-medium">$11.04920 / 0.11%</span>
          </p>
          <div className="w-full p-1 flex border rounded-lg bg-black-700 border-green-10 mt-1 items-center">
            <button onClick={decreaseValue} disabled={value <= 0.01}>
            ...
            <input
              ...
              value={value}
              onChange={handleInputChange}
              className="text-center text-white bg-transparent w-full"
            />
        ...
        <div className="w-3/12 flex flex-col gap-1">
          <p className="text-sm font-medium text-green-300 text-end">{selectedData.ask.toFixed(5)}</p>
          <button onClick={() => handleOpenDialog('buy')} className="w-full p-3 rounded-lg mt-2 bg-green-400 hover:bg-green-500 text-bodyBg">
            Buy
          </button>
        </div>
      </div>
      ...
      {/* Dialogs */}
      {isOpen && activeDialog && (
        <div className={`fixed top-0 left-0 w-full h-screen bg-black-700 bg-opacity-70 flex items-center justify-center z-50 ${isClosing ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
          <div ref={dropdownref} className={`fixed bottom-0 left-0 lg:relative ${isClosing ? 'animate-slideUp' : 'animate-slideDown'}`}>
            {activeDialog === 'buy' && <BuyDialog value={value} onClose={closeDropdown} data={selectedData} />}
            {activeDialog === 'sell' && <SellDialog value={value} onClose={closeDropdown} data={selectedData} />}
            {activeDialog === 'extand' && <Buysellextend data={selectedData} onClose={closeDropdown} dropdownref={dropdownref} />}
          </div>
        </div>
      )}
    </div>
